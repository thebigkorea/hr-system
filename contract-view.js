...
function renderContract(result) {
  ...
  if (isService) {
    ...
    document.getElementById("ogTitle")?.setAttribute("content", "사업소득자 용역계약서 완료본");
    document.getElementById("ogImage")?.setAttribute("content", "https://thebigkorea.github.io/hr-system/service-view-thumb.jpg");
  } else if (isPart) {
...
function renderServiceContract(c, signature, result) {
  return `
    <p>
      한국의집 롯데월드몰점(이하 “위탁자”라 한다)과 수탁자
      <strong>${c.empName || ""}</strong>
      (이하 “수탁자”라 한다)은 다음과 같이 용역계약을 체결한다.
    </p>
    ...
    <p>
      ...
      사업소득세 등 관계 법령에 따른 원천징수 대상 금액은 공제 후 지급한다.
    </p>
    ...
    ${signBox(c, signature, result, "위탁자", "수탁자")}
  `;
}
