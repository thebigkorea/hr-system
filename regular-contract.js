...
function initMoneyInputs() {
  ...
  ids.forEach(id => {
    ...
    input.addEventListener("input", function () {
      ...
      this.value = Number(onlyNumber || 0).toLocaleString();
      calculateTotalPay();
    });
  });
}
...
function calculateTotalPay() {
  ...
  document.getElementById("totalPay").value = total.toLocaleString();
}
...
function collectData() {
  return {
    ...
    workTime: value("startTime") && value("endTime") ? `${value("startTime")} ~ ${value("endTime")}` : "",
...
async function completeElectronicContract(event) {
  ...
  document.getElementById("signedTime").innerText = getTodayKorean() + " 전자서명 완료";
